...
class TabbedBookAdmin(BookAdmin):
    list_filter = ('id', 'name',)
    suit_tabs = (('tab1', 'Tab1'), ('tab2', ugettext('Tab2')))
    suit_templates = None
...
class TabbedFormsTestCase(ModelsTestCaseMixin, UserTestCaseMixin):
    def setUp(self):
    ...
    def test_template_includes(self):
        ...
        TabbedBookAdmin.suit_templates = (
            (suit_template_include, 'top', 'tab1'),
        )
        self.get_response(self.url)
        self.assertTemplateUsed(self.response,
                                'suit/includes/change_form_includes.html')
        self.assertTemplateUsed(self.response, suit_template_include)
